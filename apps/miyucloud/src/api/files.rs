...
pub async fn get_file(
    ...
    match db.file_by_id(&id) {
        ...
        Ok(None) => error_response(StatusCode::NOT_FOUND, "FILE_NOT_FOUND", "Le fichier demande n'existe pas."),
        Err(e) => error_response(StatusCode::INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", &e.to_string()),
    }
}
    ...
    match FileOps::download_file(db, storage, key_manager, &id) {
        Ok(data) => {
            ...
            Response::builder()
                ...
                .body(Body::from(data))
                .unwrap_or_else(|_| error_response(StatusCode::INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", "Failed to build response"))
        }
        Err(e) => match &e {
...
pub async fn upload_file(
    ...
    let Some(data) = file_data else {
        return error_response(StatusCode::BAD_REQUEST, "MISSING_FILE", "No file field in multipart request");
    };
    ...
    match FileOps::upload_file(
        ...
        Err(e) => error_response(StatusCode::INTERNAL_SERVER_ERROR, "UPLOAD_ERROR", &e.to_string()),
    }
}
...
pub async fn update_file(
    ...
    if let Some(name) = &body.name {
        if let Err(e) = FileOps::rename_file(db, &id, name) {
            return error_response(StatusCode::INTERNAL_SERVER_ERROR, "UPDATE_ERROR", &e.to_string());
        }
    }
    if let Some(pid) = &body.parent_id {
        let new_parent = if pid.is_empty() { None } else { Some(pid.as_str()) };
...
pub async fn restore_file(
    ...
    match miyucloud::domain::TrashOps::restore_file(db, &id) {
        ...
        Err(e) => error_response(StatusCode::INTERNAL_SERVER_ERROR, "RESTORE_ERROR", &e.to_string()),
    }
}
...
pub async fn search_files(
    ...
    match db.file_search(owner_id, &query.q) {
        ...
        Err(e) => error_response(StatusCode::INTERNAL_SERVER_ERROR, "SEARCH_ERROR", &e.to_string()),
    }
}
