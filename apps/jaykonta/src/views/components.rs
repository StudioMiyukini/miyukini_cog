    ...
    rsx! {
        div {
            ...
            div {
                ...
                span { style: "font-size: 13px; color: {c.text_secondary};", "{label}" }
                span { style: "font-size: 20px;", "{icon}" }
            }
            p { style: "font-size: 28px; font-weight: 600; color: {c.text_white}; margin-bottom: 4px;", "{value}" }
            span { style: "font-size: 12px; color: {detail_color};", "{detail}" }
        }
    }
}
...
pub fn AmountDisplay(amount: f64, currency: String) -> Element {
    ...
    rsx! {
        span { style: "font-weight: 500; color: {color};", "{formatted}" }
    }
}
...
pub fn ProgressBar(value: f64, max: f64, #[props(default)] color: String) -> Element {
    ...
    rsx! {
        div {
            ...
            div { style: "width: {pct}%; height: 100%; background: {bar_color}; border-radius: 4px; transition: width 0.3s;" }
        }
    }
}
        ...
        div {
            ...
            div {
                ...
                div {
                    ...
                    p { style: "font-size: 13px; color: {c.text_primary};", "{description}" }
                    if !category.is_empty() {
                        span { style: "padding: 2px 8px; background: {c.bg_secondary}; border-radius: 3px; font-size: 10px; color: {c.text_muted};", "{category}" }
                    }
                }
                p { style: "font-size: 11px; color: {c.text_muted}; margin-top: 2px;", "{date}" }
            }
            span { style: "font-size: 14px; font-weight: 500; color: {amount_color}; white-space: nowrap;", "{amount_str}" }
        }
    }
}
...
pub fn PlaceholderSection(title: &'static str, icon: &'static str) -> Element {
    ...
    rsx! {
        div {
            ...
            span { style: "font-size: 64px; margin-bottom: 16px; opacity: 0.3;", "{icon}" }
            h2 { style: "font-size: 20px; color: {c.text_secondary};", "{title}" }
            p { style: "font-size: 14px; margin-top: 8px;", "Cette section sera bientot disponible" }
        }
    }
}
