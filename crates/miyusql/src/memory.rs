...
impl MemoryExecutor {
    ...
    fn exec_sql(
        ...
        if sql_upper.starts_with("INSERT INTO") {
            let (table, cols, vals) =
                extract_insert(sql, params).unwrap_or((String::new(), vec![], vec![]));
            if table.is_empty() {
...
fn extract_table_name_create(sql: &str) -> Option<String> {
    ...
    let name = if tokens.len() >= 4
        && tokens[1].eq_ignore_ascii_case("IF")
...
impl QueryExecutor for MemoryExecutor {
    fn execute(
        ...
        let mut tables = self
            .tables
            .lock()
            .map_err(|_| MiyuSQLError::Execution("lock".into()))?;
        let mut schema = self
            .schema
            .lock()
            .map_err(|_| MiyuSQLError::Execution("lock".into()))?;
