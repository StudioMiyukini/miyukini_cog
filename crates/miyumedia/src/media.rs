...
pub fn upload(ctx: &GovernedContext, _payload: &str, _blob: Option<&[u8]>) -> Result<String, MiyumediaError> {
    if !ctx.has_mandate() {
...
pub fn serve(ctx: &GovernedContext, _media_ref: &str, _mode: Option<&str>) -> Result<Vec<u8>, MiyumediaError> {
    if !ctx.has_mandate() {
        return Err(MiyumediaError::NoMandate);
    }
    Err(MiyumediaError::Unimplemented)
...
pub fn transform(ctx: &GovernedContext, _media_ref: &str, _options: &str) -> Result<Vec<u8>, MiyumediaError> {
    if !ctx.has_mandate() {
        return Err(MiyumediaError::NoMandate);
    }
    Err(MiyumediaError::Unimplemented)
