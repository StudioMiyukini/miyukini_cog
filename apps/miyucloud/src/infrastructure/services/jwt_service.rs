...
pub struct JwtTokenService {
    /// Primary secret key used for signing JWT tokens
    jwt_secret: String,
    /// Previous secret key(s) for graceful rotation (still accepted for validation).
    /// New tokens are always signed with `jwt_secret`; previous keys only validate.
...
impl JwtTokenService {
    ...
    pub fn new(
        ...
        if !previous_secrets.is_empty() {
            tracing::info!(
                "JWT key rotation: {} previous key(s) loaded for graceful validation",
...
impl TokenServicePort for JwtTokenService {
    ...
    fn validate_token(&self, token: &str) -> Result<TokenClaims, DomainError> {
        ...
        // Try primary key first, then fall back to previous keys (rotation)
        ...
        .or_else(|primary_err| {
            ...
            for prev_secret in &self.previous_secrets {
                if let Ok(data) = decode::<JwtClaims>(
                    token,
                    &DecodingKey::from_secret(prev_secret.as_bytes()),
                    &validation,
                ) {
                    tracing::debug!("Token validated with previous (rotated) key");
                    return Ok(data);
                }
            }
            Err(primary_err)
