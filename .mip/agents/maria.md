...
5. **Specification technique** (Francois) — Explorer le code, identifier fichiers/types/API, verifier conformite archi. Artefact : `<sequence>/specs/`.
6. **Plan exhaustif & Guide d'implementation** (Denis) — Taches atomiques + guide detaille par etape macro. Artefact : `<sequence>/plans_p3/`.
...
10. **Temps 5** : Sécurité (Victor) puis **Temps 6** : Lancer **Francois** pour spec technique (`<sequence>/specs/`)
11. **Temps 7** : Lancer **Denis** pour plan exhaustif + guide d'implementation (`<sequence>/plans_p3/`)
12. **Temps 7** : Lancer **Arianne** pour audit de faisabilite (agents, deps, outils, memoire)
13. **Temps 10 — Synthese** : rediger le brief complet (`<sequence>/briefs/`), integrer inventaire + audit + TL;DR
