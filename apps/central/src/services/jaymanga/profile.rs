...
pub fn Profile(state: Signal<JayMangaState>) -> Element {
    ...
    // genres_explored et cogs_visited sont des JSON Vec<String> — on compte les éléments
    ...
    // Temps de lecture estimé (env. 30 secondes par page → pages_read * 0.5 min)
    ...
    rsx! {
        div {
            ...
            PageHeader {
                title: "👤 Profil de progression".to_string(),
            ...
            {
                // Générer les 7 derniers jours (Lun→Dim de cette semaine)
                ...
                // Calculer les jours validés : les `current_streak` derniers jours en remontant depuis aujourd'hui
                // (simplifié : on considère le streak comme continu jusqu'à aujourd'hui)
            ...
            if badges.is_empty() {
                EmptyState {
                    ...
                    message: "Lisez des manga pour débloquer des badges !".to_string(),
                    icon: "🏅".to_string(),
                }
            } else {
                div {
...

            {
                ...
                if recent_favs.is_empty() {
                    rsx! {
                        EmptyState {
                            title: "Aucune lecture récente".to_string(),
                            message: "Votre historique de lecture apparaîtra ici.".to_string(),
                            icon: "📜".to_string(),
                        }
                    }
                } else {
