...
//! Données réelles via JayMangaDb (conns.read().jaymanga).
...
/// Onglet actif de la bibliothèque lecteur.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum LibraryTab {
    #[default]
    Favorites,
...
/// État local complet de la vue JayManga.
#[derive(Debug, Clone, Default)]
pub struct JayMangaState {
    /// Section active.
    pub section: JayMangaSection,
    /// ID de l'œuvre en cours d'édition (None = création).
    pub editing_work_id: Option<String>,
    /// ID de l'œuvre dont on gère les chapitres.
    pub chapters_work_id: Option<String>,
    /// Onglet actif de la bibliothèque.
    pub library_tab: LibraryTab,
    /// ID de l'œuvre en cours de lecture (liseuse).
...
pub fn JayMangaView() -> Element {
    ...
    // Vérifier si l'onboarding est terminé (ReaderProgression.onboarding_completed)
    ...
    // Si l'onboarding n'est pas terminé, afficher l'onboarding Miou
            ...
            div {
                ...
                Onboarding {
                    on_complete: move |activated_seller: bool| {
                        ...
                        // Créer ou mettre à jour la progression avec onboarding_completed = true
                        ...
                        // Si le vendeur a été activé, créer la config boutique
                        if activated_seller {
    ...
    // Vérifier si le vendeur a une configuration boutique
                ...
                div {
                    ...
                    if !has_seller_config {
                        components::EmptyState {
                            ...
                            message: "Configurez votre boutique manga pour commencer à publier et vendre.".to_string(),
                            icon: "📚".to_string(),
