//! Module JayFestival — vue complete avec gestion multi-role.
//!
...
/// Section active cote Organisateur.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum OrgSection {
    #[default]
    Dashboard,
...
/// Onglet actif dans le hub d'une edition (Organisateur).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
...
/// Etat local complet de la vue JayFestival.
...
pub fn JayFestivalView() -> Element {
    ...
    // Si non connecte, afficher la facade publique (UNC)
    ...
    // Charger les donnees communes (utilisateur connecte)
    let editions = db.editions_list().unwrap_or_default();

    let exposants_count = db.exposants_list(false).map(|v| v.len()).unwrap_or(0);

    let pending_count = db.candidatures_pending_count().unwrap_or(0);
...
/// Vue facade publique pour utilisateurs non connectes.
#[component]
