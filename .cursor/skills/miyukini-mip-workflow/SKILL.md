...
|-------|------------------------|-------------|
| SETUP | `modules/setup.md` | `.mip/environment.md` absent ou `/mip_setup` |
| P0 | `modules/p0-details.md` | Debut de P0 (T3+) |
| P3 | `modules/p3-execution.md` | Debut de P3 |
| P4-P6 | `modules/p4-p5-p6.md` | Debut de P4 |
| Metriques | `modules/metrics.md` | Init metriques (debut sequence) |
| MASS | `modules/mass.md` | T4-T5 avec parallelisation |
| Ref. outils IA | `modules/tools-reference.md` | Sur demande explicite |

**Instruction** : Lire le module avec l'outil Read AVANT de commencer la phase correspondante. Ne PAS charger tous les modules au debut — charger uniquement le module de la phase en cours. Chemin relatif depuis ce fichier : `.cursor/skills/miyukini-mip-workflow/modules/<fichier>`.

**Tables de reference partagees** : Les tables classification T1-T5, equipe agents, artefacts MIP, et workflow standard sont dans **CLAUDE.md** (sections "Classification des taches", "Equipe dev", "Artefacts MIP", "Workflow standard"). Ne pas les dupliquer ici.

---

## Phase SETUP — Onboarding Universel (UNE SEULE FOIS)

> Detail complet : `modules/setup.md`
...
| I-12 | **Artefacts structures dans `.mip/`** | briefs/, specs/, plans/, audits/, metrics/, reports/ |
| I-13 | **Frein d'urgence** | Arret automatique si bug bloquant apres 2 tentatives ou delta majeur |
| I-14 | **Documents modulaires, 400 lignes max** | Tout artefact decoupe si depassement |
...
> Table de reference dans CLAUDE.md, section "Classification des taches".
...
> Detail de chaque Temps : `modules/p0-details.md`
...
**Regles de bornage P0** (detail dans `modules/p0-details.md` section "Regles de presentation P0") :
...
Chaque Temps est **trace via TodoWrite**. A la completion, l'agent annonce :
...
**Persistance** : `memory/user-profile.md` section "Preferences de travail". Changeable via `/autonomy_mode <mode>`.
...
> Detail complet : `modules/p3-execution.md`
...
> Detail complet : `modules/p4-p5-p6.md`
...
- **Jean** : Audit efficience tokens — consommation vs budget, anomalies, rapport dans `.mip/audits/`.
...
3 volets : Rapport final (.mip/reports/) → Archivage artefacts → Capitalisation (memory/) + Jean refactorise memoire avec Arianne.
...
2. **Anti-patterns charges** — Lire `memory/patterns-and-lessons.md` et MEMORY.md avant chaque sprint
...
9. **Adaptation automatique** — MIP adapte ses capacites (parallelisme, TodoWrite, Context7) a l'outil IA detecte
...
| **Responsable Efficience IA** (Jean) | `mip-performance-history.md`, `MEMORY.md`, `.mip/metrics/`, agents `.md` |
| **Team Manager** (Arianne) | `mip-decisions.md`, `patterns-and-lessons.md`, `mip-performance-history.md`, `team-skills-audit.md` |

> Note : Dans Miyukini COG, ces fichiers sont nommes : `rust-patterns.md`, `dioxus-cheatsheet.md`, `mscm-templates.md`. Les noms universels sont pour un nouvel environnement.
...
> Detail complet : `modules/mass.md`. Actif pour T4-T5.
...
DAG dans `.mip/dags/`. Metriques swarm dans `.mip/metrics/`.
...
Le plan exhaustif (`.mip/plans/`) sert **a la fois** de plan d'execution ET de journal de suivi. Chaque agent annote directement le plan :

```
> Demarre a HH:MM:SS. Termine a HH:MM:SS avec [model] pour N tokens (mesures).
```

**ZERO estimation** : Le `N` est la valeur reelle extraite de la task-notification de l'agent, pas une approximation. Si les tokens ne sont pas encore disponibles, ecrire `tokens: a completer` et l'orchestrateur remplit apres reception de la task-notification. Voir `modules/metrics.md` pour les methodes de collecte.
