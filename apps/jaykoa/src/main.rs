//! Point d'entree binaire JayKoa (standalone).
//!
...
fn main() {
    tracing_subscriber::fmt()
        .with_env_filter("jaykoa=debug,dioxus=info")
        .init();

    tracing::info!("Demarrage de JayKoa (standalone)...");

    let config = dioxus::desktop::Config::new()
        .with_window(
            dioxus::desktop::WindowBuilder::new()
                .with_title("JayKoa \u{2014} Calendrier Universel")
...
fn App() -> Element {
    ...
    let db_path = format!("{data_dir}/jaykoa.db");
    let db = match jaykoa::data::JayKoaDb::open(&db_path) {
        Ok(db) => Arc::new(db),
        Err(e) => {
            tracing::error!("Impossible d'ouvrir la DB JayKoa: {e}");
            return rsx! {
                div {
