//! Vue Mois generique — Grille mensuelle compacte.
//!
//! @id: central.components.calendar.month_view
//! @do: render_generic_month_calendar_view
//! @role: ui
//! @layer: ui
//! @human: Vue calendrier mois generique reutilisable

use dioxus::prelude::*;
use crate::state::use_app_state;
use chrono::{Datelike, NaiveDate};
use super::types::{CalendarEvent, WEEKDAY_NAMES_LONG};

/// Props pour la vue mois generique.
#[derive(Props, Clone, PartialEq)]
pub struct CalendarMonthViewProps<E: CalendarEvent + 'static> {
    /// Annee.
    pub year: i32,
    /// Mois (1-12).
    pub month: u32,
    /// Evenements a afficher.
    pub events: Vec<E>,
    /// Callback quand on clique sur un jour.
    #[props(default)]
    pub on_day_click: Option<EventHandler<NaiveDate>>,
    /// Callback quand on clique sur un evenement.
    #[props(default)]
    pub on_event_click: Option<EventHandler<String>>,
    /// Nombre max d'evenements affiches par jour (defaut: 3).
    #[props(default = 3)]
    pub max_events_per_day: usize,
    /// Afficher les noms de jours en format court.
    #[props(default = false)]
    pub short_weekday_names: bool,
}

/// Vue mois avec grille compacte.
#[component]
pub fn CalendarMonthView<E: CalendarEvent + 'static>(props: CalendarMonthViewProps<E>) -> Element {
    let state = use_app_state();
    let c = state.read().current_theme.palette();

    let today = chrono::Local::now().date_naive();

    // Premier jour du mois
    let first_day = NaiveDate::from_ymd_opt(props.year, props.month, 1).unwrap_or(today);
    // Decalage pour commencer le lundi
    let start_offset = first_day.weekday().num_days_from_monday() as i64;
    // Premier jour affiche
    let start_date = first_day - chrono::Duration::days(start_offset);

    // Nombre de jours dans le mois
    let days_in_month = if props.month == 12 {
        NaiveDate::from_ymd_opt(props.year + 1, 1, 1)
    } else {
        NaiveDate::from_ymd_opt(props.year, props.month + 1, 1)
    }.map(|d| d.pred_opt().unwrap_or(d).day()).unwrap_or(30);

    // Nombre de semaines a afficher (5 ou 6)
    let total_days = start_offset as u32 + days_in_month;
    let weeks = ((total_days + 6) / 7).max(5);

    // Noms des jours
    let weekday_names: Vec<&str> = if props.short_weekday_names {
        vec!["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"]
    } else {
        WEEKDAY_NAMES_LONG.to_vec()
    };

    rsx! {
        div {
            style: "display: flex; flex-direction: column; height: 100%; overflow: hidden;",

            // Header avec jours de la semaine
            div {
                style: "display: grid; grid-template-columns: repeat(7, 1fr); border-bottom: 1px solid {c.border}; background: {c.bg_secondary}; flex-shrink: 0;",

                for name in weekday_names.iter() {
                    div {
                        style: "padding: 12px; text-align: center; font-size: 12px; font-weight: 500; color: {c.text_secondary}; text-transform: uppercase;",
                        "{name}"
                    }
                }
            }

            // Grille des jours
            div {
                style: "flex: 1; display: grid; grid-template-rows: repeat({weeks}, 1fr);",

                for week in 0..weeks {
                    div {
                        key: "week-{week}",
                        style: "display: grid; grid-template-columns: repeat(7, 1fr); border-bottom: 1px solid {c.border};",

                        for day_of_week in 0..7u32 {
                            {
                                let day_index = (week * 7 + day_of_week) as i64;
                                let date = start_date + chrono::Duration::days(day_index);
                                let is_current_month = date.month() == props.month;
                                let is_today = date == today;

                                // Evenements du jour
                                let day_str = date.format("%Y-%m-%d").to_string();
                                let day_events: Vec<&E> = props.events.iter()
                                    .filter(|e| {
                                        if let Some(start) = e.start_datetime() {
                                            start.starts_with(&day_str)
                                        } else {
                                            false
                                        }
                                    })
                                    .collect();

                                let bg = if is_today {
                                    c.bg_hover.to_string()
                                } else if !is_current_month {
                                    c.bg_main.to_string()
                                } else {
                                    c.bg_card.to_string()
                                };

                                let day_color = if is_today {
                                    "white".to_string()
                                } else if !is_current_month {
                                    c.text_muted.to_string()
                                } else {
                                    c.text_primary.to_string()
                                };

                                let day_bg = if is_today {
                                    c.accent_blue.to_string()
                                } else {
                                    "transparent".to_string()
                                };

                                let day_num = date.day();
                                let more_count = day_events.len().saturating_sub(props.max_events_per_day);

                                let cursor = if props.on_day_click.is_some() { "pointer" } else { "default" };
                                rsx! {
                                    div {
                                        key: "{date}",
                                        style: "padding: 4px; border-right: 1px solid {c.border}; background: {bg}; min-height: 80px; cursor: {cursor}; overflow: hidden;",
                                        onclick: move |_| {
                                            if let Some(ref handler) = props.on_day_click {
                                                handler.call(date);
                                            }
                                        },

                                        // Numero du jour
                                        div {
                                            style: "display: flex; justify-content: flex-end; margin-bottom: 4px;",
                                            span {
                                                style: "width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; border-radius: 50%; font-size: 13px; color: {day_color}; background: {day_bg};",
                                                "{day_num}"
                                            }
                                        }

                                        // Evenements (max affiches)
                                        div {
                                            style: "display: flex; flex-direction: column; gap: 2px;",
                                            for (idx, event) in day_events.iter().take(props.max_events_per_day).enumerate() {
                                                {
                                                    let event_id = event.id().map(|s| s.to_string());

                                                    rsx! {
                                                        MonthEventPill {
                                                            key: "{date}-{idx}",
                                                            event: (*event).clone(),
                                                            on_click: move |evt: MouseEvent| {
                                                                evt.stop_propagation();
                                                                if let (Some(ref handler), Some(ref id)) = (&props.on_event_click, &event_id) {
                                                                    handler.call(id.clone());
                                                                }
                                                            },
                                                        }
                                                    }
                                                }
                                            }

                                            if more_count > 0 {
                                                span {
                                                    style: "font-size: 10px; color: {c.text_link}; padding: 2px 4px;",
                                                    "+{more_count} autres"
                                                }
                                            }
                                        }
                                    }
                                }
                            }
                        }
                    }
                }
            }
        }
    }
}

/// Pilule d'evenement pour la vue mois.
#[derive(Props, Clone, PartialEq)]
struct MonthEventPillProps<E: CalendarEvent + 'static> {
    event: E,
    on_click: EventHandler<MouseEvent>,
}

#[component]
fn MonthEventPill<E: CalendarEvent + 'static>(props: MonthEventPillProps<E>) -> Element {
    let color = props.event.color();
    let title = props.event.title();

    // Heure de debut
    let time = props.event.start_datetime()
        .and_then(|s| s.get(11..16))
        .unwrap_or("");

    rsx! {
        div {
            style: "display: flex; align-items: center; gap: 4px; padding: 2px 4px; background: {color}20; border-left: 3px solid {color}; border-radius: 2px; overflow: hidden; cursor: pointer;",
            onclick: move |evt| props.on_click.call(evt),

            if !time.is_empty() && !props.event.is_all_day() {
                span {
                    style: "font-size: 10px; color: {color}; font-weight: 500;",
                    "{time}"
                }
            }

            span {
                style: "font-size: 11px; color: {color}; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;",
                "{title}"
            }
        }
    }
}
