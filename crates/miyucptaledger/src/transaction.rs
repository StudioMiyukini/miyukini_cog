...
pub fn categorize(ctx: &GovernedContext, _transaction_ref: &str, _rules: &str) -> Result<String, MiyucptaledgerError> {
    if !ctx.has_mandate() {
...
pub fn vat_resolve(ctx: &GovernedContext, _transaction_ref: &str) -> Result<String, MiyucptaledgerError> {
    if !ctx.has_mandate() {
        return Err(MiyucptaledgerError::NoMandate);
    }
    Err(MiyucptaledgerError::Unimplemented)
