...
pub fn add(ctx: &GovernedContext, user_id: &str, blocked_id: &str) -> Result<(), MiyusocialmoderationError> {
    if !ctx.has_mandate() {
        return Err(MiyusocialmoderationError::NoMandate);
    }
    let mut guard = store::blocks().lock().map_err(|_| MiyusocialmoderationError::InvalidInput("lock".into()))?;
...
pub fn remove(ctx: &GovernedContext, user_id: &str, blocked_id: &str) -> Result<(), MiyusocialmoderationError> {
    if !ctx.has_mandate() {
        return Err(MiyusocialmoderationError::NoMandate);
    }
    if let Some(v) = store::blocks().lock().map_err(|_| MiyusocialmoderationError::InvalidInput("lock".into()))?.get_mut(user_id) {
...
pub fn list(ctx: &GovernedContext, user_id: &str) -> Result<Vec<String>, MiyusocialmoderationError> {
    if !ctx.has_mandate() {
        return Err(MiyusocialmoderationError::NoMandate);
    }
    let guard = store::blocks().lock().map_err(|_| MiyusocialmoderationError::InvalidInput("lock".into()))?;
