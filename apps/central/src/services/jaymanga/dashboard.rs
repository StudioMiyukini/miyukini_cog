...
pub fn Dashboard(state: Signal<JayMangaState>) -> Element {
    ...
    let total_revenue_cents: i64 = licenses.iter()
        ...
        .sum();
    let total_revenue = format!("{},{:02} €", total_revenue_cents / 100, total_revenue_cents % 100);
    ...
    rsx! {
        div {
            ...
            // Accès rapides (3 cartes principales)
            div {
                ...
                QuickAccessCard {
                    ...
                    count: format!("{work_count} œuvres — {draft_count} brouillons"),
                    onclick: move |_| { state.write().section = JayMangaSection::Catalogue; },
                }
                QuickAccessCard {
                    icon: "💰".to_string(),
                    title: "Mes Ventes".to_string(),
                    count: format!("{license_count} ventes — ce mois: {month_sales}"),
                    onclick: move |_| { state.write().section = JayMangaSection::Sales; },
                }
                QuickAccessCard {
                    icon: "📚".to_string(),
                    title: "Ma Bibliothèque".to_string(),
                    count: format!("{} favoris — 🔥 {} jours", favorites.len(), current_streak),
            ...
            // Œuvres populaires (par nombre de ventes)
            {
            ...
            div {
                ...
                ActionButton {
                    label: "Ajouter une œuvre".to_string(),
                    icon: "➕".to_string(),
                    accent: true,
                    onclick: move |_| {
                        let mut s = state.write();
                ...
                ActionButton {
                    label: "Portail Agrégé".to_string(),
                    icon: "🌐".to_string(),
                    onclick: move |_| { state.write().section = JayMangaSection::Aggregation; },
                }
            }
