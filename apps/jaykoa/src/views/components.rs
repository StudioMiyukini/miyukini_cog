//! Composants UI reutilisables pour JayKoa.
//!
//! - CalendarHeader : barre d'en-tete avec navigation et selecteur de vue
...
/// Barre d'en-tete du calendrier avec navigation et controles.
#[component]
pub fn CalendarHeader(props: CalendarHeaderProps) -> Element {
    let c = use_palette();

    // Formater le libelle de periode selon la vue
    ...
    rsx! {
        div {
            ...
            // Partie droite : selecteur de vue
            div {
...
/// Props pour un bloc d'evenement.
#[derive(Props, Clone, PartialEq)]
pub struct EventBlockProps {
    pub entry: TemporalEntry,
    /// Hauteur du bloc en pixels (calculee selon la duree).
    #[props(default = 60)]
    pub height_px: u32,
    /// Decalage vertical en pixels.
    #[props(default = 0)]
    pub top_px: u32,
    /// Largeur en pourcentage (pour les evenements qui se chevauchent).
    #[props(default = 100)]
    pub width_percent: u32,
    /// Decalage horizontal en pourcentage.
    #[props(default = 0)]
    pub left_percent: u32,
    /// L'evenement est en conflit.
    #[props(default = false)]
...
/// Bloc d'evenement dans la grille du calendrier.
#[component]
pub fn EventBlock(props: EventBlockProps) -> Element {
    ...
    let top_px = props.top_px;
    let left_percent = props.left_percent;
    let width_percent = props.width_percent;
    let height_px = props.height_px;

    rsx! {
        div {
            style: "position: absolute; top: {top_px}px; left: {left_percent}%; width: calc({width_percent}% - 4px); height: {height_px}px; background: {color}; border-radius: 4px; padding: 4px 8px; overflow: hidden; cursor: pointer; transition: transform 0.1s, box-shadow 0.2s; border-left: {border};",
            onclick: move |_| {
            ...
            // Heure
            if !time_label.is_empty() && height_px > 40 {
                span {
...
pub fn ConflictBadge(props: ConflictBadgeProps) -> Element {
    ...
    rsx! {
        div {
            style: "display: flex; align-items: center; gap: 8px; padding: 8px 12px; background: {badge_bg}; border: 1px solid {badge_border}; border-radius: 6px;",
