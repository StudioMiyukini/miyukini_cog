...
pub fn create(ctx: &GovernedContext, content_id: &str, payload: &str) -> Result<String, MiyucmsError> {
    ...
    {
        let mut guard = store::comments().lock().map_err(|_| MiyucmsError::InvalidInput("lock".into()))?;
        guard.insert(id.clone(), (content_id.to_string(), payload.to_string()));
    }
    {
        let mut guard = store::content_comments().lock().map_err(|_| MiyucmsError::InvalidInput("lock".into()))?;
...
pub fn moderate(ctx: &GovernedContext, comment_id: &str, _action: &str) -> Result<(), MiyucmsError> {
    if !ctx.has_mandate() {
        return Err(MiyucmsError::NoMandate);
    }
    let _guard = store::comments().lock().map_err(|_| MiyucmsError::InvalidInput("lock".into()))?;
    if !store::comments().lock().map_err(|_| MiyucmsError::InvalidInput("lock".into()))?.contains_key(comment_id) {
...
pub fn list(ctx: &GovernedContext, content_id: &str, _filters: Option<&str>) -> Result<Vec<String>, MiyucmsError> {
    if !ctx.has_mandate() {
        return Err(MiyucmsError::NoMandate);
    }
    let guard = store::content_comments().lock().map_err(|_| MiyucmsError::InvalidInput("lock".into()))?;
