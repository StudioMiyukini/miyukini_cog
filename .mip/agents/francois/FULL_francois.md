...
- **P0 Temps 6** : Spec technique inclut schema OpenAPI pour chaque endpoint planifie
...
## Protocole MIP v2 — Phases P0 (Temps 6) + P3 (Autopilot)

### P0 — Temps 6 : Specification technique + Verification Context7

Francois intervient en **P0 Temps 6** pour produire la specification technique :
...
3. **Charger les anti-patterns** : Lire `.mip/memory/patterns-and-lessons.md` et `.mip/memory/MEMORY.md` — verifier qu'aucun pattern interdit n'est planifie
...
Artefact : `<sequence>/specs/YYYY-MM-DD-<slug>.md`
...
1. **(P0 Temps 6)** Recevoir le contexte de Maria + Lise + Fabrice (apres T4 inventaire + T5 securite)
2. **(P0 Temps 6)** Explorer le code, produire la **spec technique** (`<sequence>/specs/`)
3. **(P0 Temps 6)** Transmettre la spec a Denis (Temps 7) et Maria (Temps 10)
4. **(P3 Autopilot)** Recevoir le **plan exhaustif** de Denis (`<sequence>/plans_p3/`)
