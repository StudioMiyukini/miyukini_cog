...
impl std::fmt::Display for JayFestivalSyncError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "JayFestival sync: {}", self.0)
    }
}
...
impl JayFestivalAdapter {
    ...
    pub fn sync_editions(
        koa_db: &Arc<JayKoaDb>,
        target_agenda_id: &str,
    ) -> Result<usize, JayFestivalSyncError> {
        let now = Local::now().format("%Y-%m-%dT%H:%M:%S").to_string();
        let mock = vec![
            (
                "fest_ed_2026_printemps",
                "Festival Printemps 2026",
                "2026-03-15T09:00:00",
                "2026-03-17T18:00:00",
        ...
        for (sid, title, start, end, loc) in &mock {
            ...
            koa_db
                .reflect_upsert(&e)
                .map_err(|e| JayFestivalSyncError(e.to_string()))?;
            count += 1;
        }
        Ok(count)
    }
