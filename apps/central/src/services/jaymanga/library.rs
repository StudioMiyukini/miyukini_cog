    ...
    rsx! {
        div {
            ...
            div {
                ...
                TabButton { label: "Téléchargés", count: downloaded.len(), is_active: active_tab == LibraryTab::Downloads, onclick: move |_| { state.write().library_tab = LibraryTab::Downloads; } }
                ...
                TabButton { label: "Terminés", count: completed.len(), is_active: active_tab == LibraryTab::Completed, onclick: move |_| { state.write().library_tab = LibraryTab::Completed; } }
            }
        ...
        div {
            ...
            div {
                ...
                div {
                    ...
                    if is_downloaded {
                        Badge { text: "💾 Local".to_string(), color: "#8b5cf6".to_string() }
                    }
                }
    ...
    if favorites.is_empty() {
        return rsx! {
            EmptyState {
                ...
                message: "Visitez le Portail d'un COG pour découvrir des manga et les ajouter à vos favoris.".to_string(),
                icon: "❤️".to_string(),
            }
        };
    }
    ...
    if active_licenses.is_empty() {
        return rsx! {
            EmptyState {
                ...
                message: "Les œuvres achetées apparaîtront ici avec leur licence.".to_string(),
                icon: "🎫".to_string(),
            }
        };
    }
            ...
            for license in active_licenses.iter() {
                {
                    ...
                    let amount_str = if amount == 0 { "Gratuit".to_string() } else { format!("{},{:02} €", amount / 100, amount % 100) };
...
fn DownloadsTab(favorites: Vec<jaymanga::data::ReaderFavorite>, state: Signal<JayMangaState>) -> Element {
    ...
    if downloaded.is_empty() {
        return rsx! {
            EmptyState {
                title: "Aucun téléchargement".to_string(),
                message: "Les œuvres téléchargées pour lecture hors-ligne apparaîtront ici.".to_string(),
                icon: "💾".to_string(),
            }
        };
    }
    ...
    if in_progress.is_empty() {
        return rsx! {
            EmptyState {
                ...
                message: "Les œuvres que vous avez commencé à lire apparaîtront ici.".to_string(),
                icon: "📖".to_string(),
            }
        };
    }
...
fn CompletedTab(favorites: Vec<jaymanga::data::ReaderFavorite>, state: Signal<JayMangaState>) -> Element {
    ...
    if completed.is_empty() {
        return rsx! {
            EmptyState {
                title: "Aucune œuvre terminée".to_string(),
                message: "Les œuvres lues à 100% apparaîtront ici.".to_string(),
                icon: "🏆".to_string(),
            }
        };
    }
