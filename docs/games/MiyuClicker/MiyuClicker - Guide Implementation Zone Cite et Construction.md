...
|-----------|-------------------|------|
| Ressources suffisantes | **Vert** (ex. `egui::Color32::from_rgb(0, 180, 0)` ou thème) | Actif : prélever coût et démarrer construction. |
| Ressources insuffisantes | **Blanc** (ex. `egui::Color32::WHITE` ou fond neutre) | Inactif : pas d’effet. |

Implémentation suggérée : `ui.add(egui::Button::new("Construire").fill(couleur))` avec `couleur` selon `can_afford_construction(state, building_type)` ; `sense` ou `enabled` selon la même condition pour désactiver le clic.
...
- **Largeur** : 200 px (fixe ou `ui.available_width().min(200.0)`).
- **Affichage** : `progress / required` en ratio (0..1) ou en texte « X / Y pts ».
- **Widget** : `egui::ProgressBar::new((progress / required).min(1.0) as f32).show_percentage()` ou `.text(format!("{:.0}/{:.0}", progress, required))`.
...
- **Entre** : le **header** (TopBottomPanel avec ressources, horloge, vitesses) **et** la **liste des bâtiments + boutons de clic**.
- **Dans** : le `CentralPanel` ou équivalent ; ordre des enfants : header → **zone cité** → contenu (boutons clic + cartes bâtiment).
...
Implémentation egui : récupérer la taille disponible après le header (ex. `ui.available_rect_before_wrap()` ou `ctx.available_rect()`), puis `height = (available_height * 0.20).max(200.0)` et allouer un rect de `width = available_width`, `height = height`. Dessiner dans ce rect (ciel + sol + sprites).
...
Couleurs suggérées (egui) :

- Ciel : `egui::Color32::from_rgb(135, 206, 235)` (bleu clair) ou thème.
- Sol : `egui::Color32::from_rgb(210, 180, 140)` (beige / marron clair) ou thème.

Dessin : `painter.rect_filled(rect_ciel, 0.0, color_ciel)` ; `painter.rect_filled(rect_sol, 0.0, color_sol)`.

### 2.4 Sprites personnages (3×1 px)

- **Taille** : 1 px de large, 3 px de haut (rectangle 1×3).
- **Structure** :
  - **Pixel du haut** : blanc (`Color32::WHITE`) — tête.
  - **Pixels du bas** (2 pixels) : couleur du corps selon le type.

| Type   | Corps (2 pixels) |
|--------|-------------------|
| Gens   | Vert `Color32::from_rgb(0, 128, 0)` ou équivalent. |
| Soldat | Rouge `Color32::from_rgb(180, 0, 0)` ou équivalent. |
| Maçon  | Marron foncé `Color32::from_rgb(101, 67, 33)` ou équivalent. |

Dessin : pour chaque sprite, un rect de 1×3 px à la position `(x, y)` (en coordonnées écran ou zone sol), puis 3 `painter.rect_filled` de 1×1 (ou un seul rect 1×3 avec dégradé non requis ; plus simple : 3 carrés 1×1 empilés avec les couleurs tête + corps + corps).
...
- **Algorithme** : à chaque frame (ou à intervalle de temps), pour chaque sprite :
...
- **Positions des sprites** : soit dans l’état du jeu (si sauvegardées), soit dans l’état UI de l’app (recalculées à chaque frame). Recommandation : **état UI** (ex. `Vec<(f32, f32, PersonType)>` pour positions + type), mis à jour chaque frame à partir de `state.gens`, `state.soldats`, `state.macons` (nombre de sprites) et d’un RNG ou d’un timer pour le déplacement.
...
// Ou une liste par type ; mise à jour chaque frame avec positions dans rect_sol.
```
...
1. **Bouton construction**
   ...
   - Sur chaque carte bâtiment, ajouter en deuxième ligne un bouton « Construire » : couleur vert si `can_afford_*`, blanc sinon ; `enabled` ou `sense` selon la même condition.
   ...
   - Barre de progression : s’assurer qu’elle fait 200 px de large (ou min(200, available)) et affiche bien progression/required.

2. **Zone cité (layout + ciel/sol)**
   - Dans l’écran Ma citée, après le header, allouer un rect de hauteur = 20 % (min 200 px) et largeur 100 %.
   - Dessiner le ciel (60 % haut) et le sol (40 % bas) avec les couleurs définies.
...
3. **Sprites et mouvement**
   ...
   - Initialiser ou mettre à jour les positions dans le rect du sol ; dessiner chaque sprite en 3 pixels (1 blanc, 2 corps selon le type).
   - Implémenter le déplacement aléatoire (dx/dy, mise à jour, clamp dans le sol).
...
| Mouvement | Aléatoire dans zone sol ; positions en état UI ; clamp ou rebond aux bords. |
