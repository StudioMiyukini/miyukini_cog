# Miyukini Agenda — Maquettes UI type Google Agenda

## Contexte

Ce document décrit une **copie de l’UI de Google Agenda** (Google Calendar) adaptée au contexte Miyukini Agenda : structure générale, **fonctionnalités de tri**, **filtre** et **affichage par jour / 3 jours / semaine / mois**. Les **maquettes ASCII** servent de référence pour les composants à intégrer dans les UIs des services consommateurs (JayRDV, Miyukini Festival Service).
...
| **Par source** | Grouper ou ordonner par source (JayRDV, MFS). | Liste, vue agrégée |
| **Par priorité / statut** | Si le consommateur expose un champ priorité ou statut (ex. candidat, inscrit). | Liste |

Le **tri** s’applique aux données fournies par Miyukini Agenda (paramètre de requête ou tri côté client après réception).

### 1.3 Filtres

| Filtre | Description | Zone UI type |
|--------|-------------|--------------|
| **Source** | Afficher uniquement RDV (JayRDV), ou uniquement Festivals (MFS), ou Tous. | Barre d’outils ou panneau latéral |
...
| [x] MFS (Festivals)       |                    Jour / 3j / Semaine / Mois / Liste                |
| [ ] Ateliers              |                                                                      |
...
| [x] MFS — Participations     [couleur]   |
| [x] MFS — Ateliers           [couleur]   |
| [ ] MFS — Candidatures       [couleur]   |
| ---------------------------------------- |
| Tri : [ Chronologique ▼ ]                |
|       • Chronologique                    |
|       • Par type                         |
...
| [x] Festivals              | Lun 27 jan | 12:00  | Festival X — Stand      | MFS      | Particip.|
|                             | Mar 28 jan | 14:00  | Atelier Y                | MFS      | Atelier  |
| Tri: [Chronologique v]      | Mer 29 jan | 10:00  | Festival X — Stand       | MFS      | Particip.|
|                             | ...        |        |                          |          |          |
+-----------------------------+------------+--------+-------------------------+----------+----------+
```
...
## 9. Correspondance avec les composants Miyukini Agenda

| Zone / fonctionnalité | Composant Miyukini Agenda | Données fournies par |
|-----------------------|----------------------------|----------------------|
| **Zone principale (grille)** | AGD-UI-01 (Vue calendrier jour/semaine/mois) | Miyukini Agenda — entrées pour utilisateur, période, filtres. |
| **Vue 3 jours** | AGD-UI-01 (option période = 3 jours) | Miyukini Agenda — même contrat, période = 3 jours. |
| **Vue Liste** | AGD-UI-07 (Vue liste / agenda) | Miyukini Agenda — liste chronologique. |
| **Barre filtres (source, type)** | AGD-UI-04 (Filtre par source / type) | Miyukini Agenda — liste des sources/types ; paramètres de requête. |
| **Tri** | Paramètre de requête ou tri côté client | Miyukini Agenda — entrées ; tri chronologique par défaut (date début). |
| **Navigation (date, Aujourd’hui)** | Données période ; MiyuClock (date courante) | Service consommateur + Miyukini Agenda (entrées pour nouvelle période). |
| **Export** | AGD-UI-03 (Bloc export iCal, PDF) | Miyukini Agenda — export selon Mandat. |
| **Prochaine entrée** | AGD-UI-05 (Indicateur prochaine entrée) | Miyukini Agenda — prochaine entrée. |
...
| Document | Rôle |
|----------|------|
| [Miyukini Agenda - Ecrans et UI](../Miyukini%20Agenda%20-%20Ecrans%20et%20UI.md) | Composants AGD-UI-01 à 10, filtres détaillés. |
| [Miyukini Agenda - Referentiel Fonctionnel Inspire Google Agenda](./Miyukini%20Agenda%20-%20Referentiel%20Fonctionnel%20Inspire%20Google%20Agenda.md) | Référentiel Google Agenda (vues, partage, etc.). |
| [Miyukini Agenda - Operateurs et Toolkits](../Miyukini%20Agenda%20-%20Operateurs%20et%20Toolkits.md) | Contrat données (période, filtres, tri). |

---

**Document** : Miyukini Agenda — Maquettes UI type Google Agenda  
**Version** : 1.0  
**Date** : 2026-01-31  
