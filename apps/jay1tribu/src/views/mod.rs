...
//!
//! Migre depuis apps/central/src/services/jay1tribu/mod.rs.
//! - DB via crate::use_db() au lieu de Central ServiceConnections
...
#[component]
pub fn Jay1TribuView(profile_id: Signal<String>) -> Element {
    let db = crate::use_db();

    let pid = profile_id.read().clone();
    let cog_id = pid.clone();
    ...
    let profile_id_eff = pid.clone();
    let cog_id_eff = cog_id.clone();
    use_effect(move || {
        // En standalone, pas de MwsViewState : online_ids vide.
        // Sera connecte a un vrai mecanisme de decouverte plus tard.
        let online_ids: Vec<String> = Vec::new();
        let _ = process_pending_deliveries(&db_eff, &cog_id_eff, &online_ids);
        let friends_list =
            get_friends_with_presence(&db_eff, &profile_id_eff, &online_ids).unwrap_or_default();
        friends.set(friends_list);
        tribes.set(db_eff.tribe_list(&cog_id_eff).unwrap_or_default());
        salons.set(db_eff.salon_list_for_cog(&cog_id_eff).unwrap_or_default());
    });
            ...
            div { style: layout_style,
                ...
                div { style: right_style,
                    ...
                    if selected_id.is_some() {
                        ...
                        p { style: "margin-top: 12px; color: #6b7280; font-size: 12px;", "Barre de saisie a brancher (send_message)." }
                    } else {
