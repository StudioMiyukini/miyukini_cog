...
> **Obsolète — Archivé le 2026-02-17.** Ancien emplacement : `docs/ux_ui/Miyukini - Stack UI egui eframe.md`.  
> La stack UI officielle pour les applications desktop est désormais **Dioxus** (apps/central). egui/eframe ont été retirés du projet. Ce document est conservé à titre de référence historique.

## Contexte (historique)

Miyukini adoptait **egui** et **eframe** comme stack UI native pour les applications desktop (Hub de Services, outils d'administration, clients locaux). Ce choix permet un **pur Rust**, une **licence permissive** (MIT ou Apache-2.0), et une couverture **desktop + web + Android** (et embedded ultérieurement) sans contrainte commerciale ni attribution obligatoire.
...
- Le **Hub** est un **Opérateur d'Interface** (Strate 7) : il expose le catalogue de Services (Registre d'Opérateurs) et permet de lancer des Services. L'UI Hub etait implementee en **egui/eframe** (remplacé par Dioxus).
- **Pas de logique metier dans l'UI** : l'app envoie des **intentions** vers le COG ; BondingBrother, StrongFather, Master Butler gouvernent.
- **Licence** : egui/eframe sous MIT ou Apache-2.0.

---

## 9. Bonnes pratiques

- **Persistence** : activer la feature `persistence` et implementer `App::save` pour restaurer taille/position des fenetres et etat minimal.
- **Repaint** : si une tache longue ou un thread met a jour l'etat, appeler **`ctx.request_repaint()`**.
...
**Archivé le :** 2026-02-17  
**Statut :** Obsolète — stack remplacée par Dioxus (apps/central).
