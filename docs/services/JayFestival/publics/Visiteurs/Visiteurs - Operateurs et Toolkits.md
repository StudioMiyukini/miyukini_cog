...
Ce document décrit les **besoins en Opérateurs** (Strate 7) et en **Toolkits** (Strate 6) du public **Visiteurs** du service JayFestival. Il s’appuie sur le [Parcours, capacités et services](./Visiteurs%20-%20Parcours%20Capacites%20Services.md) et les documents associés. Il vise à fournir une référence produit pour l’architecture gouvernée : quels Opérateurs exposent les capacités aux visiteurs, et quels Kits d’outils agrègent les outils sous-jacents.
...
### 2.1 Opérateur « JayFestival Visiteur » (espace dédié visiteur)
...
### 2.2 Opérateur « JayFestival Catalogue » (découverte — partagé avec utilisateur non connecté)
...
| **Lien avec JayFestival Visiteur** | Le visiteur **découvre** via JayFestival Catalogue puis **réserve / s’inscrit** via JayFestival Visiteur (ou parcours d’inscription à un atelier, achat billet, etc., selon services activés par l’organisateur). |

### 2.3 Synthèse des Opérateurs (public Visiteurs)

| Opérateur | Usage par le visiteur | Livrables couverts |
|-----------|------------------------|---------------------|
| **JayFestival Visiteur** | Connexion, agenda, billets, réservations, pass VIP, suivi d’activités (jeux, concours, ateliers). | Espace dédié visiteur, compte cross-événements, conflits de dates. |
| **JayFestival Catalogue** | Découverte des événements, organisateurs, exposants ; recherche. | Catalogue (annuaire, répertoires) — partagé avec utilisateur non connecté. |

---

## 3. Besoins en Toolkits (public Visiteurs)

### 3.1 Kit « Agenda Visiteur » (JayFestival)
...
| **Consommé par** | JayFestival Visiteur. |
| **Composants sous-jacents** | MiyuClock, Miyubooking (ou équivalent), KindMother. |

### 3.2 Kit « Billets & Réservations » (JayFestival)
...
| **Consommé par** | JayFestival Visiteur. |
| **Composants sous-jacents** | KindMother ; paramétrage par JayFestival Édition (organisateur). |

### 3.4 Kit « Suivi d’activités » (JayFestival)
...
### 3.5 Kit « Catalogue » (JayFestival) — voir Utilisateur non connecté
...
| **Consommé par** | JayFestival Catalogue (partagé avec utilisateur non connecté). |
...
|---------|-----------------------------|---------------------|
| **Agenda Visiteur** | JayFestival Visiteur | Agenda personnel, compte à rebours, conflits de dates. |
| **Billets & Réservations** | JayFestival Visiteur | Billets, réservations (ateliers, créneaux), annulation/modification. |
| **Pass VIP** | JayFestival Visiteur | Pass VIP, avantages. |
| **Suivi d’activités** | JayFestival Visiteur | Jeux, concours, ateliers, récompenses, historique. |
| **Catalogue** | JayFestival Catalogue | Annuaire événements, répertoires, recherche. |
...
|----------------------|-----------|------------|
| Onboarding (inscription visiteur, par festival ou groupe) | Miyauth, Miyuprofile ; JayFestival Visiteur | Équipe & Permissions (Miyauth), Agenda Visiteur. |
| Découverte (catalogue) | JayFestival Catalogue | Catalogue. |
| Réservation / Inscription (atelier, concours, billet) | JayFestival Visiteur | Billets & Réservations, Agenda Visiteur (conflit check). |
| Organisation (agenda, billets, pass) | JayFestival Visiteur | Agenda Visiteur, Billets & Réservations, Pass VIP. |
| Participation (jeux, concours, ateliers) | JayFestival Visiteur | Suivi d’activités, Billets & Réservations. |
| Conflits de dates | JayFestival Visiteur | Agenda Visiteur (conflict.check). |
